...
구현되고 실제로 동작 검증된 상태 기준 명세서. 스택: Flask + Flask-SQLAlchemy(MySQL) + OpenAI API + GitHub REST API. 세션 기반 인증.
...
| password | String(50) | 평문 저장 (데모 수준, 프로덕션 부적합) |
...
### POST /signup
Body: `{ username, password, github_username }` → `201` / `400`(필드 누락)

### POST /signin
Body: `{ username, password }` → `200 { username, password }` (세션 생성) / `404`(존재하지 않는 유저)
...
1. `compare_portfolios(repo1, repo2, field)`로 두 저장소를 같은 컨텍스트에서 7개 항목 10점 만점 상대 평가
...
## 6. LLM 분석 모듈 (`llm_analyzer.py`)

OpenAI API(`response_format: json_schema`, `strict: true`, `temperature=0`)로 구조화된 채점 결과를 강제해서 받는다. LLM 호출 전 GitHub REST API로 각 저장소의 README 존재 여부, 언어, 최상위 파일 목록, CI/배포 설정 파일 존재 여부, 테스트 디렉터리 존재 여부, 커밋/릴리즈 존재 여부를 수집해 컨텍스트로 첨부한다 (LLM이 URL을 직접 크롤링하지 않음).

- `analyze_portfolio(repository, field)` — 저장소 하나를 7개 항목 절대 채점 (포트폴리오 등록/수정용)
- `compare_portfolios(repo1, repo2, field)` — 두 저장소를 한 프롬프트에서 상대 비교 (대결용). 개별 분석 점수를 재사용하지 않고 매 대결마다 새로 평가 — 절대 채점의 미세한 편차가 상대 승패를 왜곡하지 않도록 함
- `rank_for_score(score)`, `rank_tier_diff(rank_a, rank_b)`, `RANK_SCORE_GAIN` — 포트폴리오 랭크와 매칭 시스템이 공유하는 랭크 계산 유틸

**환경변수**: `OPENAI_API_KEY`(필수), `GITHUB_TOKEN`(선택, 없으면 GitHub API가 시간당 60회로 제한됨 — 저장소 하나 분석에 API 호출 ~6회 소요되므로 테스트 중 금방 소진될 수 있음), `ANALYSIS_MODEL`(기본 `gpt-4o-mini`).

**오류 처리**: JSON 스키마 검증 실패 시 최대 1회 재시도 후 `LLMError` → 라우트에서 `502`. GitHub 조회 실패(비공개/404/rate limit 등)는 `AnalysisError` → `422`.

---

## 7. 알려진 한계

- 비밀번호 평문 저장, 세션 시크릿 하드코딩 — 데모용이며 프로덕션 배포 전 반드시 수정 필요
