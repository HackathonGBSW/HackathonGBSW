...
| player_rank_score | Integer, default 0 | 플레이어 랭크 점수 (최저점에서 누적, 24단계 티어 산정에 사용) |
...
**플레이어 랭크** (`player_rank`, `llm_analyzer.player_tier_for_score`) — 상한 없이 누적되는 `player_rank_score`를 24단계 사다리로 매긴다: 브론즈4→3→2→1 → 실버4→3→2→1 → 골드4→3→2→1 → 플래티넘4→3→2→1 → 다이아몬드4→3→2→1 → 신입 → 주니어 → 미들 → 시니어. 브론즈~다이아몬드는 각 티어당 20점씩(디비전당), 신입 이상은 80점씩. 정확한 구간표는 `llm_analyzer.PLAYER_TIERS` 참고.

- 대결 결과: 승자 `+8 + min((연승 횟수-1)*2, 10)`(연승 보너스, 6연승 이상부터 +10 상한 → 총 +18) / 패자 `-3`(0 미만으로는 내려가지 않음, 연승 0으로 리셋) / **무승부는 랭크 점수·승패·연승 모두 변동 없음**
- 매칭(`POST /battles/match`)의 "랭크 차이 최대 1단계"는 이제 24단계 사다리 기준(`player_tier_diff`)으로 판정 — 기존 7단계(S~F) 기준보다 훨씬 촘촘한 매칭이 이뤄진다.
...
```json
{
  ...
  "player_rank": {
    "material": "bronze|silver|gold|platinum|diamond|newbie|junior|middle|senior",
    "division": "4|3|2|1|null",
    "label": "string (예: \"골드 3\", \"시니어\")",
    "index": 0,
...
```
- `player_rank`는 플레이어 랭크 사다리(`llm_analyzer.player_tier_for_score`, 브론즈4~1 → 실버4~1 → 골드4~1 → 플래티넘4~1 → 다이아몬드4~1 → 신입 → 주니어 → 미들 → 시니어, 총 24단계) 기준. `portfolio_best_rank`/`portfolio_avg_rank`는 기존 포트폴리오 분석 등급(S~F) 그대로 유지.
...
- Query: `field`(선택) — 지정 시 해당 주분야 사용자만.
- `200 [ { username, main_field, player_rank, player_rank_score, battle_win, battle_lose, win_streak }, ... ]` (player_rank_score 내림차순)
